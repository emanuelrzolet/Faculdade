...
CREATE TABLE Livro (
    ...
    CONSTRAINT idEditora_FK FOREIGN KEY (idEditora) REFERENCES Editora(idEditora)
);
...
CREATE TABLE ItemPedido (
    ...
    CONSTRAINT itemIdPedido_FK FOREIGN KEY (idPedido) REFERENCES Pedido(idPedido),
    CONSTRAINT itemIdLivro_FK FOREIGN KEY (idLivro) REFERENCES Livro(idLivro)
);
...
-- Consultas
-- Implementação 2:
SELECT COUNT(idLivro) AS totalLivrosCadastrados
FROM Livro;
...
-- Implementação 4:
SELECT E.nome, L.titulo FROM Editora E INNER JOIN Livro L ON  E.idEditora = L.idEditora ORDER BY E.nome DESC;

-- Implementação 5:
SELECT E.nome AS Editora, AVG(L.preco) AS Média_Preço FROM Editora E
INNER JOIN Livro L ON E.idEditora = L.idEditora
GROUP BY E.nome;

-- Implementação 6:
SELECT C.nome, COUNT(ip.idLivro) AS Titulos_Comprados
FROM Cliente C
